...
- Phase 8 now includes a recursion-first flagship lesson: Maximum Depth of Binary Tree is implemented as a registered, verified recursive DFS lesson centered on an execution tree, with an optional call-stack aid and a checked-in golden under `content/lessons/maximum-depth/approaches/recursive-dfs/goldens`
...
- Phase 8 now includes a memoization flagship lesson: Coin Change Memo DFS is implemented as a registered, verified memoized recursion lesson with a call-tree primary surface plus memo-table and call-stack aids, and a checked-in golden under `content/lessons/coin-change/approaches/memo-dfs/goldens`
...
- Phase 9 now verifies dense presets at the shell-region level: stage and auxiliary rails stay within the desktop viewport contract, author review remains docked, and secondary aids never collapse code/reference space
...
- Maximum Depth of Binary Tree now treats the recursive execution tree as the only default stage surface, with the call stack available only as a secondary aid
...
- Graph BFS is now the first structural-node token pilot: shared `current` and `neighbor` execution tokens project across graph nodes, frontier queue items, traversal state, and narration without treating pointer styling as the semantic source of truth
...
- the learner-facing task switch has been replaced with a dedicated problem-selector modal on `E`, backed by a typed lesson catalog with category, difficulty, mechanism, confusion, and flagship-status metadata
